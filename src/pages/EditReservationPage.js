...
function EditReservationPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setMessage("RESERVATION MODIFIEE AVEC SUCCES");
  ...
  const confirmDeleteReservation = async () => {
    try {
      ...
      setMessage("RESERVATION SUPPRIMEE");
      setMessageType("success");
      await loadReservations();
      setViewMode("list");
      setSelectedResa(null);
  ...
  const handleValidation = async (id, statut) => {
    try {
      ...
      const msg =
        statut === 1
          ? "RESERVATION VALIDEE PAR LES RH"
          : "RESERVATION REFUSEE PAR LES RH";
      setMessage(msg);
          ...
          >
            ...
            <div style={{ display: "flex", gap: 40, flexWrap: "wrap" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 16, fontWeight: 600, color: "#333" }}>
                  {selectedResa.Modello}
                </div>
              </div>
              <div>
                <span style={{ fontSize: 12, color: "#888" }}>Plaque</span>
                <div
                  style={{ fontSize: 18, fontWeight: 700, color: "#2E75B6" }}
                >
                  {selectedResa.Targa}
                </div>
              </div>
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: "#333" }}>
                {selectedResa.UtentePrenotazione}
              </div>
            </div>
            <div
              style={{ background: "#f9f9f9", borderRadius: 8, padding: 16 }}
            ...
            >
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: "#333" }}>
                ...
                {formatDate(selectedResa.DataFine)} ({selectedResa.Giorni} jour
                {selectedResa.Giorni > 1 ? "s" : ""})
              </div>
            </div>
            <div
              style={{ background: "#f9f9f9", borderRadius: 8, padding: 16 }}
  ...
  <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
    <div style={{ background: "#fff", borderRadius: 8, padding: 30, maxWidth: 450, width: "90%", textAlign: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
      <div style={{ fontSize: 40, marginBottom: 16 }}>⚠️</div>
      <h3 style={{ margin: "0 0 12px", color: "#1a3a5c" }}>Confirmer la suppression</h3>
      <p style={{ color: "#666", marginBottom: 24 }}>Etes-vous sur de vouloir supprimer cette reservation ? Cette action est irreversible.</p>
      <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
        <button onClick={() => setConfirmDelete(null)} style={{ background: "#f0f0f0", color: "#333", border: "none", borderRadius: 6, padding: "10px 30px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>ANNULER</button>
        <button onClick={confirmDeleteReservation} style={{ background: "#dc3545", color: "#fff", border: "none", borderRadius: 6, padding: "10px 30px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>SUPPRIMER</button>
      </div>
    </div>
  </div>
)}
